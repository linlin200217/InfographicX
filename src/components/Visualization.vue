...
// 接收来自父组件的 visualization 数据（来源于 knowledge.visualization）
...
const drawChart = () => {
  ...
  // 获取容器尺寸
  const containerWidth = chartContainer.value.clientWidth || 60;
  const containerHeight = chartContainer.value.clientHeight || 60;
  ...
  // 计算图表的有效绘制区域（容器中间80%）
  const effectiveWidth = containerWidth * 0.90;
  const effectiveHeight = containerHeight * 0.90;
  ...
  if (type === "pie_chart") {
    ...
    const radius = (Math.min(effectiveWidth, effectiveHeight) / 2) * 0.80;
    // 在有效区域中居中显示饼图
    ...
    //arcs.append("text")
      ...
      const svgBar = chartGroup.append("g")
      ...
      const chartWidth = effectiveWidth - margin.left - margin.right;
      const chartHeight = effectiveHeight - margin.top - margin.bottom;
      const svgLine = chartGroup.append("g")
                      .attr("transform", `translate(${margin.left},${margin.top})`);
      ...
      const yAxis = d3.axisLeft(y)
                      ...
                      .tickSizeOuter(0)
                      .tickFormat(() => "");
      const yAxisGroup = svgLine.append("g")
          .call(yAxis);
  }
...
// 当 visualization 数据变化时，重新绘制图表
watch(() => props.visualization, () => {
  drawChart();
}, { deep: true });
</script>
